...
def run_multihead_self_attention(
    ...
    mhsa = MultiheadSelfAttention(
        d_model, num_heads, use_rope=False, device=in_features.device)
    mhsa.qkv_project.weight.data = torch.cat(
...
def run_multihead_self_attention_with_rope(
    ...
    from cs336_basics.multihead_self_attention import MultiheadSelfAttention
    mhsa = MultiheadSelfAttention(
        d_model, num_heads, use_rope=True, theta=theta, max_seq_len=max_seq_len, device=in_features.device)
    mhsa.qkv_project.weight.data = torch.cat(
        [q_proj_weight, k_proj_weight, v_proj_weight], dim=0)
    mhsa.out_proj.weight.data = o_proj_weight
...
def run_rope(
    ...
    rpe = RotaryPositionalEmbedding(
        theta, d_k, max_seq_len, device=in_query_or_key.device)
    return rpe(in_query_or_key, token_positions)
    raise NotImplementedError
